...
"""Il serait meilleur dans notre cas d'utiliser une classe afin d'éviter les définitions globales, économiser de la RAM / du CPU et simplifier le code"""

def placer(event = None): #fonction executée quand il y a un clic sur le cnavas
    global cases
    X = (event.x - event.x%tailleCase) + tailleCase/2 #on récupère les coordonées de la cellule cliquée
    Y = (event.y - event.y%tailleCase) + tailleCase/2

    if cases[X,Y] == "":
        cases[X,Y] = joueur[1] #si la calse était vide, on la remplis
        verifier()
...
def jouer(event = None): #fonction principale
    ...
    tour +=1
    Main["text"]= joueur[0]
    fenetre.unbind("<Return>") #on enlève le bind de la touche entrée à la fenetre

def verifier(): #fonction de vérification si quelqu'un a aligné trois cases
    global Main
    global score
    win = None #variable pour vérifier si quelqu'un a gagné
    #lignes et colones
    ...
    for (x,y),symbole in cases.items(): #affichage des cases cliquées
        if symbole == "O":
    ...
    if win != None or tour == 9: #si qqn a gagné ou si toutes les cases sont remplies, on arrete la partie
        if win != None: #si il y a quelqu'un qui a gagné
            partie[win] += 1 #on augmente le nombre de parties gagnées du joueur vainqueur
        ...
        if answer == "yes": #si l'utilisateur veut recommencer, on régénère l'affichage
            init(True)
        else:
...
def init(bis = False): #fonction pour initialiser
    ...
    if bis == True: #si la partie a été construite en recommencant, on réinitialise les affichages et on détruit le cnvas
        Main["text"] = ""
        dessine.destroy()

    tour = 0
    dessine = Canvas(fenetre, height = 3*tailleCase, width = 3*tailleCase, background = "lightgrey") #création du canvas
...
#dictionnaire avec les coordonnées et les etats
    ...
    fenetre.bind("<Return>", jouer) #on bind la touche entrée pour commencer la partie
    fenetre.update()

fenetre = Tk() #fenetre
tailleCase = 100 #taille de la case, elle définit toutes les dimensions
